...
\subsection{Cautions and warnings}
A performance graph can only be viewed after a successful mixing run has been executed with a defined protocol (see \ref{sec:defineMixProt}). The graph option is not available after single step simulations.
...
\subsection{Procedures}
\subsubsection{Preparation}
Before the mixing performance of multiple mixing runs can be compared, at least two mix results need to be saved. See \ref{sec:savmixrun} for the tutorial on how to save a mixing result.
