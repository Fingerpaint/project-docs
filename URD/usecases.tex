...
%Define mixing protocol (3)
 \section{Define mixing protocol (3)}
  ...
  \begin{tabular}{ p{0.5\textwidth} p{0.5\textwidth} }
    ...
    11. Repeats steps 5, 7 \& 9 until satisfied. & 12. Repeats steps 6, 8 \& 10 accordingly.\\
    13. Taps on the \emph{Mix Now} button. & 14. Displays a \emph{How many times do you want to execute the protocol?} message.\\
    15. Selects a value for the number of iterations. & \\
    16. Taps the \emph{Confirm} button. & 17. Computes the result of applying all movements in the protocol-log to the initial concentration distribution for the specified number of times, and displays it on the canvas.\\
  \end{tabular}
  \newpage
      \noindent\textbf{Alternative 1}:\\
      \begin{tabular}{ p{0.5\textwidth} p{0.5\textwidth} }
  	\emph{Actor actions:} & \emph{\projectname\ response:} \\
 & 15. Fails to connect to server. Displays a \emph{Connection Failed} message.\\
    ...
    \end{tabular}
     \\
    \\If the optional steps are followed, the following alternative is applicable:
           \\\textbf{Alternative 2}: \\
        \begin{tabular}{ p{0.5\textwidth} p{0.5\textwidth} }
  	\emph{Actor actions:} & \emph{\projectname\ response:} \\
    9. (Optional) Taps the \emph{No} button. & 10. (Optional) Displays the mixing interface. \\
    11. Repeats steps 5, 7 \& 9 until satisfied. & 12. Repeats steps 6, 8 \& 10 accordingly.\\
    13. Taps on the \emph{Mix Now} button. & 14. Displays a \emph{How many times do you want to execute the protocol?} message.\\
    15. Selects a value for the number of iterations. & \\
    16. Taps the \emph{Confirm} button. & 17. Computes the result of applying all movements in the protocol-log to the initial concentration distribution for the specified number of times, and displays it on the canvas.\\
