...
The user can select a circle or square-shaped drawing tool to draw with. He can draw on the canvas by swiping with his finger or drawing the mouse.In addition, the user can reset the drawn distribution to completely white by using a \texttt{reset} button.
...
\fpstartparagraph{} To indicate how many times the protocol created using the above items is executed, the user can enter a number in a (different) spinner. We use a spinner because the number it contains can easily be incremented or decremented by using the arrow keys, and a value that is not near the original value can easily be entered using its text box.
...
%CPR33
\SRQ[CP-21]{should have}{The \texttt{export performance} option can be used to open the \texttt{export} menu to \texttt{export} an image of the current performance graph.}
%CPR35&37
\SRQ[CP-22]{could have}{The \texttt{export animation} button can be used to open the \texttt{export} menu to \texttt{export} an animation of the mixing procedure.}
%CPR39
...
%CPR33&35&37&39
\SRQ[CP-23]{should have}{The user should be able to cancel loading files by clicking outside of the loading popup.}
%CPR33&35&37&39
...
\SRQ[nomemoryexport]{should have}{If there is no memory space to store \texttt{exported/saved} data when \srqref{exportsave} is executed, the application returns an \texttt{out of memory} message.}
...
\SRQ[SS-3]{must have}{The Simulator Service must be able to pass (references to) matrix files to the Fortran Module, which the Fotran Module can use for simulations.}
