...
\SRQ{must have}{The HTTP Server must be able to serve files to the Client Browser.}

\subsection[AS]{Application Service} % Oude Mixer Provider
\SRQ{must have}{The Application Service must be able to retrieve all the available mixers from the Application Persistence.}
\SRQ{must have}{The Application Service must be able to send all the available mixers to the Client Browser.}
\SRQ{must have}{The Application Service must allow for adding new mixers to the Application Persistence.}
\SRQ{must have}{The Application Service must allow for editing existing mixers in the Application Persistence.}
\SRQ{must have}{The Application Service must allow for removing existing mixers to the Application Persistence.}
\SRQ{must have}{The Application Service must be able to provide all available geometries to the Client Browser.}

\subsection[APC]{Application Persistence Communication}
\SRQ{must have}{The Application Persistence Communication must be able to access all the data from the Application Persistence.}
\SRQ{must have}{The Application Persistence Communication must be \todo{fast enough} to serve the data from the Application Persistence to the Application Service.}

\subsection[AP]{Application Persistence} % Voorheen MixerPersistence
\SRQ{must have}{The Application Persistence must deliver all saved mixers to the Application Service.}
\SRQ{must have}{The Application Persistence must save new mixers handed by the Application Service.}
\SRQ{must have}{The Application Persistence must save changes to existing mixers handed by the Application Service.}
\SRQ{must have}{The Application Persistence must remove existing mixers handed by the Application Service.}

\subsection[ASC]{Application Service Communication} % Voorheen MixerProviderCommunication
\SRQ{must have}{The Application Service Communication must be able to access all the functionality from the Application Service.}
\SRQ{must have}{The Application Service Communication must be \todo{fast enough} to provide the functionality from the Application Service to the Client Browser.}
...
\SRQ{must have}{The mixing interface contains a cell browser with four columns, to select an initial mixer, geometry and initial distribution.} % CPR1
...
\SRQ{must have}{The user can select a blank canvas after choosing a geometry and mixer, by choosing the \texttt{blank} option in the third column in the cell browser.} % CPR6
...
\SRQ{must have}[shapedraw]{The mixing interface contains a button that displays the current shape of the drawing tool.} % CPR7
\SRQ{should have}{The button from \srqref{shapedraw} also displays the current size of the drawing tool.} % CPR10
...
When the client receives the JSON object containing the concentration distribution and the performance result from the server, the client un-JSON's these items. The concentration distribution is visualised on the client's screen, using the canvas that was originally used to draw the initial concentration distribution. The performance result is visualised in a graph, which contains all previous performance results for this particular run.

\SRQ[visResults]{must have}{ % CPR29e
    Concentration distributions can be drawn on the client's canvas.
}

\SRQ{must have}{ % CPR29f
...
\SRQ{should have}{ % CPR24
    After the user has selected the geometry and the mixer using \srqref{selgeomrec}, \srqref{selgeomsq}, \srqref{selgeomcir} or \srqref{selgeomjb} and \srqref{selmixer}, they can choose to load a previously saved mixing protocol. This spawns a menu with the names of all applicable saved protocols. Pressing the name of one such protocol loads it.
}

\SRQ{could have}{ % CPR28
    After \srqref{selgeomrec}, \srqref{selgeomsq}, \srqref{selgeomcir} or \srqref{selgeomjb} and \srqref{selmixer}, the user can load a predefined mixing protocol. This spawns a menu containing all suitable predefined mixing protocols for the selected geometry. Pressing the name of one such protocol selects it, which means it will be used once the mixing is executed.
}
...
\SRQ{should have}{The mixing interface has a \texttt{Remove saved distributions} button to remove previously saved distributions.} % CPR12
...
In the results window a \texttt{save} button, an \texttt{export} button and a \texttt{load results} button is available. If the \texttt{export} button is pressed, a popup with several available export options is shown: \texttt{export performance}, \texttt{export picture} and \texttt{export animation}. \texttt{export animation} is only available if the \texttt{animate mixing} checkbox was checked in the mixing interface. The user can then name the file and browse to a desirable storage location. If during saving the specified name for saving/exporting is already in use, the application returns a \texttt{name already in use} message. Additionally, a \texttt{load performance} button is available in the results window. The user can use this button to open a popup. In this popup he can select the files he wants to load. The button changes to \texttt{load selected} and now loads all the selected files. If no files are selected, the button is named \texttt{cancel} and will simply cancel without loading. An alternative way to cancel loading is to click somewhere not on the popup.

\SRQ{should have}{The \texttt{export picture} option can be used to open the export menu for an image of the mixing result.}
%CPR33
\SRQ{should have}{The \texttt{export performance} option can be used to open the export menu for an image of the current performance graph.}
%CPR35&37
\SRQ{could have}{The \texttt{export animation} button can be used to open the export menu for an animation of the mixing procedure somewhere on the device.}
%CPR39
\SRQ[exportname]{should have}{In the export menu, The user can specify a name for the exported file.}
%CPR33&35&37&39
\SRQ[exportlocation]{should have}{In the export menu, The user can specify a location for the exported file.}
%CPR33&35&37&39
\SRQ[exportsave]{should have}{If the user clicks on the save button in the export menu, the file is stored at the location specified in \srqref{exportlocation} with the name specified in \srqref{exportname}.}
%CPR33&35&37&39
...
\SRQ{should have}{The popup opened by \srqref{nomemoryexport} can be closed with an \texttt{ok} button}
%CPR33&35&37&39
\SRQ{should have}{The user can overwrite the old file in \srqref{nameinuse} with an \texttt{overwrite} button if a saved file already exists.}
%CPR33&35&37&39, Use case alternative S.3.1
\SRQ{should have}{The user can cancel overwriting the old file in \srqref{nameinuseexport} with a \texttt{cancel} button if a saved file already exists.}
%CPR33&35&37&39, Use case alternative S.3.1
...
\SRQ{should have}{The \texttt{load results} button can be used to open the load menu for a texttt{results} file.}
%CPR30&36
...
%CPR30&36
\SRQ{should have}{If in \srqref{displayload} multiple files are selected, all performance data is loaded in one graph.}
%CPR36
\SRQ{should have}{If in \srqref{displayload} multiple files are selected, the user is able to browse through the result mixtures.}
%CPR36, extention for consistency.
\SRQ{should have}{In the load menu, the \texttt{cancel} button can be used to exit the loading menu.}
%CPR30&36
\SRQ{should have}{If multiple files are selected in the loading menu, they are all depicted in a single performance graph.}
%CPR36
...
\SRQ{must have}{The Simulator Service Communication must be able to access all the functionality from the Application Persistence.}
\SRQ{must have}{The Simulator Service Communication must be \todo{fast enough} to serve the data from the Application Persistence to the Client Browser.}
...
\SRQ{must have}{In the results window the result of the user's mixing protocol is shown. This interface can be reached by pressing the \texttt{start mixing} button.}
%CPR32
...
%CPR34
\SRQ{must have}{The user can use the \texttt{show result} button to switch back to the standard results window and view the result distribution of the mixing run.}
%CPR34, extention for consistency.

\SRQ{could have}{An animation of the mixing run is shown in the results window if the \texttt{animate mixing} checkbox was checked.}
%CPR38

\subsection[SS]{Simulator Service}
\SRQ[execMixing]{must have}{ %CRP29c2
     The server executes the mixing using the parameters received via the socket. The concentration distribution and mixing protocol are given as input to the \textsc{Fortran} implementation.
}

\SRQ{must have}{The Simulator Service must be able to receive matrix files from the Client Browser.}
\SRQ{must have}{The Simulator Service must be able to receive protocol information from the Client Browser.}
\SRQ{must have}{The Simulator Service must be able to pass matrix files to the Fortran Module.}
\SRQ{must have}{The Simulator Service must be able to pass protocol information to the Fortran Module.}
\SRQ{must have}{The Simulator Service must make sure that that the listed mixers in the Application Persistence are known to the Fortran Module, for each geometry.}
...
\SRQ{must have}{The Fortran Module must accept input (matrix, protocol information) from the Simulator Service.}
\SRQ{must have}{The Fortran Module must provide output (vectors) to the Simulator Service.}
